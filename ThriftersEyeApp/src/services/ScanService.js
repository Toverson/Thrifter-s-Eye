...
export class ScanService {
  // Use the same backend API as web version for consistency
  static baseUrl = 'https://your-backend-url.com'; // This will be configured for production
  ...
  static async getUserScans(limitCount = 10) {
    try {
      ...
      const response = await fetch(`${this.baseUrl}/api/history?user_id=${encodeURIComponent(user.uid)}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  static async getScanById(scanId) {
    try {
      ...
      const response = await fetch(`${this.baseUrl}/api/scan/${scanId}?user_id=${encodeURIComponent(user.uid)}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
        },
